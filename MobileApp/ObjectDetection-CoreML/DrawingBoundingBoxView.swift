...
class DrawingBoundingBoxView: UIView {
    ...
    static private var colors: [String: UIColor] = ["fresh orange":UIColor(hue: 0.3, saturation: 1, brightness: 1, alpha: 0.8), "rotten orange": UIColor(hue: 0.6, saturation: 1, brightness: 1, alpha: 0.8)]
    ...
    func createLabelAndBox(prediction: VNRecognizedObjectObservation) {
        ...
        if let label = labelString, let confidence = confidence {
            // Format the confidence as a percentage string with 2 decimal places
            let confidencePercentage = String(format: "%.2f%%", confidence * 100)
            // Concatenate the label and confidence
            combinedString = "\(label) \(confidencePercentage)"
//            print(combinedString)
        } else {
            print("Label or confidence is nil")
        }
